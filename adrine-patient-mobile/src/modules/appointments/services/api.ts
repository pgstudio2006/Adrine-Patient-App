...
export class ApiAppointmentService implements IAppointmentService {
  ...
  private get baseUrl(): string {
    ...
    if (!url) throw new Error('EXPO_PUBLIC_DOMAIN_API_URL is not configured');
    return url;
  }
  ...
  async rescheduleAppointment(id: string, date: string, time: string): Promise<Appointment | undefined> {
    try {
