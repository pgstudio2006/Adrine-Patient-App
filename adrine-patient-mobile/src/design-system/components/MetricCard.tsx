import { View, Text } from 'react-native';
import React from 'react';
import { Card } from './Card';

interface MetricCardProps {
  label: string;
  value: string;
  unit?: string;
  change?: { value: string; direction: 'up' | 'down' | 'neutral' };
  icon?: React.ReactNode;
  status?: 'normal' | 'warning' | 'critical';
}

const statusColors: Record<string, string> = {
  normal: 'text-green-600',
  warning: 'text-amber-600',
  critical: 'text-red-600',
};

const statusBg: Record<string, string> = {
  normal: 'bg-green-50',
  warning: 'bg-amber-50',
  critical: 'bg-red-50',
};

export function MetricCard({ label, value, unit, change, icon, status }: MetricCardProps) {
  return (
    <Card padding="md" className={status ? statusBg[status] : ''}>
      <View className="flex-row items-start justify-between gap-2">
        <View className="flex-1">
          <Text className="text-xs text-muted font-medium" numberOfLines={1}>{label}</Text>
          <View className="flex-row items-baseline gap-1 mt-0.5">
            <Text className={`text-2xl font-bold ${status ? statusColors[status] : 'text-gray-900'}`}>
              {value}
            </Text>
            {unit && <Text className="text-xs text-muted">{unit}</Text>}
          </View>
          {change && (
            <Text className={`text-xs font-medium mt-0.5 ${
              change.direction === 'up' ? 'text-green-600' :
              change.direction === 'down' ? 'text-red-600' :
              'text-muted'
            }`}>
              {change.direction === 'up' ? '↑' : change.direction === 'down' ? '↓' : '→'} {change.value}
            </Text>
          )}
        </View>
        {icon && <View className="text-muted">{icon}</View>}
      </View>
    </Card>
  );
}
